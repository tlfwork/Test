...
namespace FairyGUI
{
    ...
    public class UIPackage
    {
        ...
        public static UIPackage AddPackage(string assetPath, LoadResource loadFunc)
        {
            ...
            pkg._assetPath = assetPath;//MyBag
        ...
        bool LoadPackage(ByteBuffer buffer, string assetNamePrefix)
        {
            ...
            buffer.version = buffer.ReadInt();//7

            bool ver2 = buffer.version >= 2;  //true

            buffer.ReadBool();                //compressed

            id = buffer.ReadString();         //g4zu7pyd

            name = buffer.ReadString();       //MyBag
            ...
            //iz4i0 BagComponent / cw4q2 n3_jg82 p_1 n5_tihw n5 aaa n6_tihw n6 tihw5 n7_tihw a_1
            //AttributeComponent n0_tihw n0 生命值:50 100 atlas0 atlas0.png
            ...
            if (assetNamePrefix.Length > 0)
            {
                ...
                assetNamePrefix = assetNamePrefix + "_";    //MyBag_
            }

            else
                assetPath = string.Empty;

            cnt = buffer.ReadShort();       //5个组件资源 处理资源组件 new PackageItem
            ...
            for (int i = 0; i < cnt; i++)   
            {
                ...
                pi.type = (PackageItemType)buffer.ReadByte();//Component         //Atlas

                pi.id = buffer.ReadS();                     //iz4i0              //atlas0

                pi.name = buffer.ReadS();                   //BagComponent       //null

                buffer.ReadS();  

                pi.file = buffer.ReadS();                   //null               //MyBag_atlas0.png

                pi.exported = buffer.ReadBool();            //true               //false

                pi.width = buffer.ReadInt();                //500                //512

                pi.height = buffer.ReadInt();               //500                //256
                ...
                switch (pi.type)
                {
                    ...
                    case PackageItemType.Component:
                        {
                            int extension = buffer.ReadByte();//0
                            ...
                            pi.rawData = buffer.ReadBuffer();       //包括资源组件自身的布局
            ...
            cnt = buffer.ReadShort();       //2个图片组件资源 记录图片资源和纹理集的对应关系
            ...
            for (int i = 0; i < cnt; i++)
            {
                ...
                pi = _itemsById[buffer.ReadS()];    //纹理集的packageitem
