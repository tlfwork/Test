...
namespace YooAsset.Editor
{
    ...
    public abstract class TaskCreateManifest
    {
        ...
        protected void CreateManifestFile(BuildContext context)
        {
            ...
            // 创建补丁清单文本文件
            {
                string fileName = YooAssetSettingsData.GetManifestJsonFileName(buildParameters.PackageName, buildParameters.PackageVersion);//PackageManifest_FirstPackage_v1.json

                string filePath = $"{packageOutputDirectory}/{fileName}";//D:/unitylearn/Test/Bundles/StandaloneWindows64/FirstPackage/v1/PackageManifest_FirstPackage_v1.json
        ...
        private List<PackageAsset> GetAllPackageAsset(BuildMapContext buildMapContext)
        {
            ...
            {
                BuildAssetInfo[] assetInfos = bundleInfo.GetAllManifestAssetInfos();
        ...
        private List<PackageBundle> GetAllPackageBundle(BuildMapContext buildMapContext)
        {
            ...
            foreach (BuildBundleInfo bundleInfo in buildMapContext.Collection)
            {
                PackageBundle packageBundle = bundleInfo.CreatePackageBundle();
