...
namespace YooAsset.Editor
{
    public class TaskGetBuildMap
    {
        ...
        public BuildMapContext CreateBuildMap(BuildParameters buildParameters)
        {
            ...
            // 1. 获取所有收集器收集的资源
            CollectResult collectResult = AssetBundleCollectorSettingData.Setting.GetPackageAssets(buildMode, packageName);
            ...
            foreach (var collectAssetInfo in allCollectAssets)
            {
                ...
                BuildAssetInfo buildAssetInfo = new BuildAssetInfo(collectAssetInfo.CollectorType, collectAssetInfo.BundleName, collectAssetInfo.Address, collectAssetInfo.AssetInfo);
            ...
            // 4. 录入所有收集资源依赖的其它资源
            foreach (CollectAssetInfo collectAssetInfo in allCollectAssets)
            {
                string bundleName = collectAssetInfo.BundleName;

                foreach (AssetInfo dependAsset in collectAssetInfo.DependAssets)
                {
                    ...
                    {
                        BuildAssetInfo buildAssetInfo = new BuildAssetInfo(dependAsset);
            ...
            foreach (CollectAssetInfo collectAssetInfo in allCollectAssets)
            {
                List<BuildAssetInfo> dependAssetInfos = new List<BuildAssetInfo>(collectAssetInfo.DependAssets.Count);
            ...
            if (collectResult.Command.AutoCollectShaders)
            {
                ...
                foreach (BuildAssetInfo buildAssetInfo in allBuildAssetInfos.Values)
                {
            ...
            // 10. 构建资源列表
            List<BuildAssetInfo> allPackAssets = allBuildAssetInfos.Values.ToList();
        ...
        {
            // 1. 检测依赖资源收集器是否存在
            if (!allCollectAssets.Exists(x => x.CollectorType == ECollectorType.DependAssetCollector))
                return;
            ...
            foreach (var collectAsset in allCollectAssets)
            {
                ...
                if (collectorType == ECollectorType.MainAssetCollector || collectorType == ECollectorType.StaticAssetCollector)
                {
                    ...
                    {
                        if (!allDependAsset.Contains(dependAsset.AssetPath))
