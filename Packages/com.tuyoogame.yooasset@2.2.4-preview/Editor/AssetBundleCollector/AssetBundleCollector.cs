    ...
    public class AssetBundleCollector
    {
        ...
        public List<CollectAssetInfo> GetAllCollectAssets(CollectCommand command, AssetBundleCollectorGroup group)
        {
            ...
            if (AssetDatabase.IsValidFolder(CollectPath))
            {
                ...
                //Assets/Res/Model/Mat
                //Assets/Res/Model/Mat/100.mat
            ...
            foreach (string assetPath in findAssets)
            {
                ...
                if (!command.IgnoreRule.IsIgnore(assetInfo) && IsCollectAsset(group, assetInfo))
                {
                    if (!result.ContainsKey(assetPath))
                    {
                        CollectAssetInfo collectAssetInfo = CreateCollectAssetInfo(command, group, assetInfo);

                        result.Add(assetPath, collectAssetInfo);//Assets/Res/Model/Mat/100.mat
                    }
                    else
                    {
            ...
            {
                Dictionary<string, string> addressTemper = new Dictionary<string, string>();

                foreach (var collectInfoPair in result)
                {
                    if (collectInfoPair.Value.CollectorType == ECollectorType.MainAssetCollector)
                    {
                        string address = collectInfoPair.Value.Address;//100

                        string assetPath = collectInfoPair.Value.AssetInfo.AssetPath;//Assets/Res/Model/Mat/100.mat
        ...
        private CollectAssetInfo CreateCollectAssetInfo(CollectCommand command, AssetBundleCollectorGroup group, AssetInfo assetInfo)
        {
            ...
            //firstpackage_assets_res_model_mat.bundle    FirstPackage_unityshaders.bundle
