...
namespace YooAsset
{
    public abstract class DownloaderOperation : AsyncOperationBase
    {
        ...
        private long _lastDownloadBytes = 0;    //上一次下载进度有变的时候的大小

        private int _lastDownloadCount = 0;

        private long _cachedDownloadBytes = 0;  //已经下载完成 并且拷贝到了cache中的下载器总下载大小
        ...
        internal override void InternalOnUpdate()
        {
            ...
            if (_steps == ESteps.Loading)
            {
                ...
                foreach (FSDownloadFileOperation downloader in _downloaders)
                {
                ...
                // 移除已经完成的下载器（无论成功或失败）
                foreach (FSDownloadFileOperation downloader in _removeList)
                {
                ...
                if (_bundleInfoList.Count > 0 && _failedList.Count == 0)
                {
                    ...
                    if (_downloaders.Count < _downloadingMaxNumber)
                    {
                        ...
                        BundleInfo bundleInfo = _bundleInfoList[index];

                        FSDownloadFileOperation downloader = bundleInfo.CreateDownloader(_failedTryAgain, _timeout);
